...
{
#if DEBUG
    public HashSet<T> alreadyRecycled = new HashSet<T>();
#endif
...
#endif
        DBC.Check.Assert(_pools.ContainsKey(pool) == true, "Cannot recycle object without being preallocated or used");
    ...
    protected void InternalRecycle(T obj, string poolName)
    {
        ...
        if (alreadyRecycled.Add(obj) == false)
            throw new Exception("An object already Recycled in the pool has been Recycled again");
#endif
        DBC.Check.Assert(_namedPools.ContainsKey(poolName) == true, "Cannot recycle object without being preallocated or used");
    ...
    public List<ObjectPoolDebugStructureString> DebugNamedPoolInfo(List<ObjectPoolDebugStructureString> debugInfo)
    {
        ...
        for (var enumerator = _namedPools.GetEnumerator(); enumerator.MoveNext();)
            debugInfo.Add(new ObjectPoolDebugStructureString(enumerator.Current.Key, enumerator.Current.Value.Count));
